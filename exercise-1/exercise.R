...
ggplot(data = diamonds) + geom_point(mapping = aes(x = carat, y = price), color="blue")
...
ggplot(data = diamonds.sample) + geom_line(mapping = aes(x = carat, y = price, color=cut))

# That's kind of messy. Try using `smooth` geometry instead.
ggplot(data = diamonds.sample) + geom_smooth(mapping = aes(x = carat, y = price, color=cut))
...
ggplot(data = diamonds.sample) + geom_histogram(aes(x=price, fill=clarity))
...
diamond.summary <- diamonds %>%  group_by(clarity) %>% 
